...
const Utility = require('./scrape-utilities.js');
 ...
 const scrape = async () => {
    ...
    try {
        await driver.get('https://www.odessa-tx.gov/AgendaCenter/City-Council-7');
        let element = await driver.findElement(By.id('table7'));
        ...
        for (let i = 0; i < rows.length; i++) {
			...
			date = row[0].querySelector('strong').text;
			
			date = `${Utility.processNamedMonthDate(date)} ${meetingTime}`;
			
			link = 'https://www.odessa-tx.gov' + row[0].querySelectorAll('a')[1].getAttribute('href');
